...
List<Person> aliceFriends = PersonToPersonsWithRelation(alice, allRelations, RelationType.friend);

Person aliceSpouse = PersonToPersonsWithRelation(alice, allRelations, RelationType.spouse).First();
List<Person> aliceSpouseFriends = PersonToPersonsWithRelation(aliceSpouse, allRelations, RelationType.friend);
...
List<Person> carolFriends = PersonToPersonsWithRelation(carol, allRelations, RelationType.friend);

Person carolSpouse = PersonToPersonsWithRelation(carol, allRelations, RelationType.spouse).First();
List<Person> carolSpouseFriends = PersonToPersonsWithRelation(carolSpouse, allRelations, RelationType.friend);
...
List<Person> onlyFriends = AllPersonsWithRelation(allRelations, RelationType.friend)
    .Except(AllPersonsWithRelation(allRelations, RelationType.spouse))
    .ToList();

Person notInvited = onlyFriends.Except(commonFriends).Except(allFriends).First();
Person datingNotInvited = PersonToPersonsWithRelation(notInvited, allRelations, RelationType.friend).First();
...
allPersons.ForEach(GenerateAndPrintFamilyTreeNode);
#endregion
...
{
    return allRelations.Aggregate(new FamilyTreeNode(p), (ftn, rel) => ftn.ExtendNodeFromRelation(rel));
}
